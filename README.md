...
The clique won on the three most important components for the circuit to me (identifiability, locality, and richness). To explain where this structure lacked, its lower type and NT coherence scores were annotation limitations as the dominant cell type (lLN1_bc) falls below the FlyWire NT classifier's confidence threshold for 23 of 38 neurons. Additionally, 36 of 38 clique neurons are ALLN (Antennal Lobe Local Neurons) from the same ALl1_dorsal hemilineage, which leads to a type coherence of approximately 18/20.

Apart from this, the star's higher type and NT coherence scores are structural limitations as all internal edges flow from the single CT1 hub to disconnected leaves, so the "coherence" reflects the hub's properties replicated across 1,876 neurons that share no connections with each other. With its circuit richness at 0, the star cannot be a functional circuit regardless of its other coherence scores.
